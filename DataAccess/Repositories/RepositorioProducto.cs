...
namespace DataAccess.Repositories
{
    public class RepositorioProducto : RepositorioMaestro, IRepositorioProducto
    {
        ...
        public RepositorioProducto()
        {
            ...
            insert = "insert into TB_PRODUCTO values(@IdCompra, @IdMarca, @IdModelo, @IdCategoria, @Codigo, @Img, @Titulo, @Caracteristicas, @Precio, @Coste, @Margen, @PVP, @IVA, @Descuento, @Total)";
            update = "update TB_PRODUCTO set IdCompra=@IdCompra, IdMarca=@IdMarca, IdModelo=@IdModelo,IdCategoria=@IdCategoria, Codigo=@Codigo, Img=@Img, Titulo=@Titulo, Caracteristicas=@Caracteristicas, Precio=@Precio, Coste=@Coste, Margen=@Margen, PVP=@PVP, IVA=@IVA, Descuento=@Descuento, Total=@Total  where IdProducto=@IdProducto";
        ...
        public IEnumerable<TB_PRODUCTO> GetAll()
        {
            ...
            foreach (DataRow item in tableResult.Rows)
            {
                listProducto.Add(new TB_PRODUCTO
                {
                    ...
                    Precio= decimal.Parse(item[9].ToString()),
                    Coste = decimal.Parse(item[10].ToString()),
                    Margen = decimal.Parse(item[11].ToString()),
                    PVP= decimal.Parse(item[12].ToString()),
                    IVA = decimal.Parse(item[13].ToString()),
                    Descuento = decimal.Parse(item[14].ToString()),
                    Total = decimal.Parse(item[15].ToString()), 
                });
            }
            return listProducto;
